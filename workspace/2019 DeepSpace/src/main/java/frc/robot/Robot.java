...
public class Robot extends TimedRobot {
    ...
    private static final int kButtonBack = 9;
    ...
    */
    ...
    public void teleopPeriodic() {
        ...
        if (m_stick.getRawButtonPressed(kButtonStart)) { // This code doesn't exist in autoPeriodic, for good reason
            macroIndex++;
            if (macroIndex == 1)
                m_walker.RaiseRobot();
            if (macroIndex == 2)
        ...
        if (m_stick.getRawButtonPressed(kButtonBack)) { // If we need to restart the climbing process.
            m_walker.RetractBackLegs();  // The restart may be impossible at macroIndex 2
            m_walker.RetractFrontLegs();
            m_walker.StraightenLegs(); // New function that is the opposite of Walk()
            macroIndex = 0;
        }
